...
foreach ($dir in $menuDirs) {
    $filePath = Join-Path $baseDir "$dir\index.html"
    if (Test-Path $filePath) {
        ...
        Write-Host "Updated footer in $dir/index.html"
    } else {
        Write-Host "Warning: Could not find $dir/index.html"
    }
}

Write-Host "Updated footer content in all menu section pages successfully!"
